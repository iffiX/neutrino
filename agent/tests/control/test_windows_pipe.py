...
the real ``_ControlRequestHandler``: the request bytes go in as a connected
client, the platform reads the peer from the connection's ``pipe_handle``,
and the JSON answer lands in the fake's write buffer.
"""
...
class FakePipeApi:
    ...
    def create_instance(self, pipe_name):
        self.created.append(pipe_name)
        self.next_handle += 1
...
class RecordingPlatform(FakeControlPlatform):
    """The control platform fake, remembering each peer connection."""

    def __init__(self):
        FakeControlPlatform.__init__(self)
        self.peers = []

    def read_peer_identity(self, connection):
        self.peers.append(connection)
        return FakeControlPlatform.read_peer_identity(self, connection)
