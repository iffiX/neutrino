"""The bridge: exactly three request fields cross, and scope never does.

The page hands the shell ``{id, method, path, body}``; the bridge forwards
method, path and body over the channel and nothing else, so whatever a page
adds to its message cannot reach the agent, and the reply's scope is the
channel's alone.
"""

from neutrino_agent.gui.bridge import GuiBridge, GuiWindowApi
...
class FakeGuiChannel:
    ...
    def __init__(self, *, status=200, reply=None, error=None):
        self.status = status
        self.reply = reply if reply is not None else {"caller": {"account": "alice"}}
        self.error = error
        self.asked = []
...
def test_a_get_round_trips_with_its_id():
    ...
    assert reply == {"id": 7, "status": 200, "body": {"caller": {"account": "alice"}}}
...
def test_scope_comes_from_the_channel_not_from_the_page():
    # A body naming another account rides through as data; the reply is the
    # channel's own scope, untouched by anything the page sent.
