"""The agent's GUI page, loaded from the frontend files the package ships.

The page is plain HTML, CSS and JavaScript under ``agent/frontend/``, copied
into ``neutrino_agent/data/gui/`` by the packaging builds; a checkout with no
built copy reads the source directory directly. The page renders three
sections — Status, Modules, Services — in the scope its channel owns: the
shell that opens the window holds the already-authenticated control
connection, and the page's requests ride it over a message bridge, so the
...
The agent reports errors and states as ``{"code", "params"}``; every word on
this surface lives in the page's own table.
"""
...
def gui_dir() -> pathlib.Path:
    ...
    Returns:
        ...
        ``agent/frontend`` source directory otherwise.
    ...
    for directory in (GUI_DATA_DIR, GUI_SOURCE_DIR):
        if (directory / "index.html").is_file():
            return directory
    raise FileNotFoundError("the agent package carries no GUI page")
...
def window_icon_path() -> str:
    ...
    """
    for path in (
        GUI_DATA_DIR / "neutrino_agent.png",
