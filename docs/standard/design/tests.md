...
| `platforms/` | The contract itself: capability advertisement, honest `unsupported_platform` refusals from the darwin and windows stubs, platform-owned account floors, account homes resolved from the account database and never `$HOME`, the step-down environment carrying the target's identity, CIFS mount options (ownership mapping under the asking account's home), tooling installs. |
| `modules/` | The package state machine: verify is dpkg's own status word (`rc` reads as absent), removal purges, both unconfirmed latches (an install or removal that ran but did not verify waits instead of retrying every recheck), the never-asked module that is reported and not touched. OpenSSH's enable/disable semantics, including installing the server first where the recommendation was skipped. Downloads and package-kind dispatch. |
...
| `web/` | Routers through a test client: the agent wire (both version gates and the generation gate, per-account key grant and revocation, a locked vault degrading instead of failing a beat), the Origin belt on state-changing panel routes, session auth, every `{code, params}` refusal shape, and the models the frontend mirrors. |
