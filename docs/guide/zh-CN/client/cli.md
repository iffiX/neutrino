...
| `service ai show`         | `--hub <name>` 看这一个 hub 的网关，省略则看目标 hub 的                                                                                                                                                                                                                                    | 这个账户的工具现在指向哪里                                    |
| `service ai apply`        | `hub` 或 `off`。模型参数有 `--claude-default`、`--claude-opus`、`--claude-sonnet`、`--claude-haiku`、`--codex-model` 和 `--gemini-model`；省略则保持现值，传 `''` 回到网关默认。`--codex-effort` 取 `minimal`、`low`、`medium`、`high` 或 `''`。`--hub <name>` 在应用前把这个 hub 设为目标 | `hub` 把工具指向目标 hub 的网关，`off` 还原                   |
