...
| 改过面板端口后要重新登录                 | 会话 cookie 的名字带端口，`neutrino_session_<port>` | 在新端口上重新登录                                                                  |
...
| `nagent status` 写着 `protocol_too_old` | 被控端说的协议号低于 hub 接受的下限                              | 在那台机器上装 hub 自带的被控端包，或在抽屉里点 **安装被控端**（Install agent）            |
| `nagent status` 写着 `protocol_too_new` | 被控端说的协议号高于 hub                                         | 先升级 hub；绑定还在，机器会自己连回来                                                     |
| 被控端日志里有 `hub_untrusted`          | hub 重置或重装过，证书变了                                       | 用一条新链接 `sudo nagent join` 重新接入                                                   |
...
| `protocol_too_new`                                | 客户端说的协议号高于 hub，先升级 hub，绑定还在             |
| `protocol_too_old`                                | 客户端说的协议号低于 hub 接受的下限，升级客户端            |
