...
def control_stack(tmp_path_factory):
    ...
    yield server, resident, platform
    server.stop()


@pytest.fixture
def control(control_stack):
    server, resident, platform = control_stack
    FakeResident.__init__(resident, platform=platform)
    FakeClientPlatform.__init__(platform)
    return server, resident, platform
...
def test_service_actions_reach_the_resident_with_their_body(control):
...
def test_a_handler_exception_answers_typed_and_keeps_the_server_answering(control):
    server, resident, _platform = control
    resident.service_error = OSError("a secret-bearing message")
...
def test_a_platform_without_a_socket_does_not_bind():
    ...
    lines = []
    server = ControlServer(
        resident=FakeResident(), platform=NoSocketPlatform(), log=lines.append
    )
