...
from neutrino_hub.web.routers.agent import file as device_files
...
def test_a_listing_is_the_agents_entries_directories_first(api):
    ...
    answer = client.get("/api/agent/file", params={"device_id": MAC, "path": "/srv"})
...
def test_a_device_with_no_channel_answers_409(api):
    ...
    answer = client.get("/api/agent/file", params={"device_id": MAC, "path": "/"})
...
def test_a_file_download_streams_the_bytes_with_the_announced_size(api):
    ...
    answer = client.get(
        "/api/agent/file/download", params={"device_id": MAC, "path": "/srv/a.bin"}
    )
...
def test_a_directory_download_is_an_archive_asked_for_as_one(api):
    ...
    answer = client.get(
        ...
        params={"device_id": MAC, "path": "/srv/.dots"},
    )
...
def test_an_upload_is_one_file_pushed_through_the_stream(api):
    ...
    answer = client.post(
        ...
        data={"device_id": MAC, "path": "/srv/in"},
...
def test_an_operation_with_no_channel_is_409(api):
    ...
    answer = client.post(
        "/api/agent/file/directory/create", json={"device_id": MAC, "path": "/x"}
    )
