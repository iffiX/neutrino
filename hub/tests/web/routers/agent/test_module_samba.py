...
from neutrino_hub.web.routers.agent import module_samba as samba_router
from tests.web.module_api_box import DEVICE, HOST, OFFLINE, module_box

BASE = "/api/agent/module/samba"
CONFIG = {
    "shares": [
...
def test_the_status_route_carries_the_live_parts_alone(box):
    ...
    payload = client.get(f"{BASE}/status", params={"device_id": DEVICE}).json()
...
def test_saving_shares_checks_on_the_agent_stores_and_pushes(box):
    ...
    response = client.post(
        f"{BASE}/share/set",
        json={"device_id": DEVICE, "shares": [{"name": "media", "path": "/srv/media"}]},
    )
...
def test_a_share_the_agent_refuses_is_refused_and_nothing_is_stored(box):
    ...
    response = client.post(
        f"{BASE}/share/set",
        json={"device_id": DEVICE, "shares": [{"name": "global", "path": "/srv/x"}]},
    )
...
def test_removing_a_user_scrubs_it_from_the_shares_that_named_it(box):
    ...
    response = client.post(
        f"{BASE}/user/set", json={"device_id": DEVICE, "users": ["bob"]}
    )
...
def test_an_offline_device_cannot_be_edited(box):
    ...
    response = client.post(f"{BASE}/user/set", json={"device_id": OFFLINE, "users": []})
...
def test_a_password_lands_only_on_a_configured_user(box):
    ...
    good = client.post(
        f"{BASE}/user/password/set",
        json={"device_id": DEVICE, "name": "ann", "password": "s3cret"},
    )
    bad = client.post(
        f"{BASE}/user/password/set",
        json={"device_id": DEVICE, "name": "ghost", "password": "x"},
    )
...
def test_a_password_samba_refuses_is_answered_with_the_agents_code(box):
    ...
    response = client.post(
        f"{BASE}/user/password/set",
        json={"device_id": DEVICE, "name": "ann", "password": "x"},
    )
...
def test_apply_pushes_the_state_again(box):
    ...
    payload = client.post(f"{BASE}/apply", json={"device_id": DEVICE}).json()
