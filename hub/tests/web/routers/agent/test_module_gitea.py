...
from neutrino_hub.web.routers.agent import module_gitea as gitea_router
from tests.web.module_api_box import DEVICE, HOST, OFFLINE, module_box

BASE = "/api/agent/module/gitea"
...
def test_a_port_the_agent_refuses_is_refused_and_nothing_stored(box):
    ...
    response = client.post(f"{BASE}/set", json={"device_id": DEVICE, "listen_port": 80})
...
def test_a_password_reset_rides_the_command(box):
    ...
    response = client.post(
        f"{BASE}/admin/password/set",
        json={"device_id": DEVICE, "username": "ann", "password": "pw2"},
    )
...
def test_an_offline_device_takes_no_verb(box):
    ...
    edit = client.post(f"{BASE}/set", json={"device_id": OFFLINE, "listen_port": 3000})
    admin = client.post(
        f"{BASE}/admin/add",
        json={
            "device_id": OFFLINE,
