...
def test_no_overlay_reads_as_none(box):
    ...
    assert client.get("/api/hub/overlay").json()["provider"] == OVERLAY_NONE
...
def test_leaving_every_overlay_writes_no_row_and_stands_the_engines_down(box):
    ...
    payload = client.post(
        "/api/hub/overlay/set", json={"provider": OVERLAY_NONE}
    ).json()
...
def test_choosing_the_stored_overlay_still_makes_the_machine_agree(box):
    ...
    client.post("/api/hub/overlay/set", json={"provider": OVERLAY_NETBIRD})
...
def test_the_row_is_written_before_the_engine_is_started(box):
    client, runtime = box
    client.post("/api/hub/overlay/set", json={"provider": OVERLAY_NONE})
    FakeSwitcher.refusal = OSError("the daemon refused")

    response = client.post("/api/hub/overlay/set", json={"provider": OVERLAY_NETBIRD})
...
def test_an_overlay_nobody_runs_is_refused(box):
    ...
    response = client.post("/api/hub/overlay/set", json={"provider": "tailscale"})
...
def test_an_engine_this_hub_does_not_run_yet_is_refused(box, monkeypatch):
    ...
    response = client.post("/api/hub/overlay/set", json={"provider": OVERLAY_EASYTIER})
...
def test_switching_to_the_other_engine_writes_its_row(box):
    ...
    payload = client.post(
        "/api/hub/overlay/set", json={"provider": OVERLAY_EASYTIER}
    ).json()
