...
def test_the_view_says_the_box_does_not_address_its_own_ports(guest_box):
    ...
    assert client.get("/api/hub/network").json()["is_addressing_owned"] is False
...
def test_closing_every_interface_is_allowed(guest_box):
    ...
    response = client.post("/api/hub/network/set", json=_options(exposed=[]))
...
def test_an_interface_this_machine_does_not_have_cannot_be_opened(guest_box):
    ...
    response = client.post("/api/hub/network/set", json=_options(exposed=["enp9s0"]))
...
def test_the_view_lists_every_overlay_this_box_is_a_member_of(box, monkeypatch):
    ...
    overlays = client.get("/api/hub/network").json()["overlays"]
...
def test_an_overlay_closes_and_reopens_through_the_same_write(box):
    ...
    closed = client.post(
        "/api/hub/network/set", json=_options(exposed=["enp1s0"], overlays=[])
    )
    assert closed.status_code == 200
    assert runtime.network().exposed_overlay_device_names == []

    reopened = client.post(
        "/api/hub/network/set", json=_options(exposed=["enp1s0"], overlays=["netbird"])
    )
...
def test_a_mode_that_is_not_one_of_them_is_refused(box):
    ...
    response = client.post("/api/hub/network/mode/set", json={"mode": "one_arm_router"})
...
def test_becoming_a_router_keeps_the_roles_a_router_had(box):
    ...
    client, runtime, _ = box
    client.post("/api/hub/network/mode/set", json={"mode": "server"})

    client.post("/api/hub/network/mode/set", json={"mode": "router"})
...
def test_becoming_a_router_drops_the_upstream_router_a_side_gateway_left(
    ...
    client, runtime, _ = guest_box
    client.post("/api/hub/network/mode/set", json={"mode": "side_gateway"})
...
def test_becoming_a_server_hands_the_machine_back(box, monkeypatch):
    ...
    response = client.post("/api/hub/network/mode/set", json={"mode": "server"})
...
def test_becoming_a_side_gateway_joins_the_network_it_is_already_on(guest_box):
    ...
    response = client.post("/api/hub/network/mode/set", json={"mode": "side_gateway"})
...
def test_taking_the_machine_over_reads_the_wifi_it_already_knew(
    ...
    client.post("/api/hub/network/mode/set", json=_becoming_an_owner())
...
def test_a_stored_passphrase_never_comes_back_out(box):
    ...
    payload = client.get("/api/hub/network/wifi_network").json()
...
def test_a_network_whose_key_is_elsewhere_says_so(box):
    ...
    network = client.get("/api/hub/network/wifi_network").json()["networks"][0]
...
def test_the_preferred_network_is_listed_first(box):
    ...
    names = [
        entry["ssid"]
        for entry in client.get("/api/hub/network/wifi_network").json()["networks"]
    ]
...
def test_forgetting_a_network_removes_it(box, monkeypatch):
    ...
    response = client.post("/api/hub/network/wifi_network/leave", json={"ssid": "home"})
...
def test_forgetting_one_the_box_does_not_know_is_a_404(box):
    ...
    assert (
        client.post(
            "/api/hub/network/wifi_network/leave", json={"ssid": "never-seen"}
        ).status_code
        == 404
    )
...
def test_a_scan_on_a_radio_nobody_applied_starts_its_supplicant(idle_radio):
    ...
    payload = client.get("/api/hub/network/interface/wifi/scan?name=wlp3s0").json()
...
def test_a_scan_on_a_running_supplicant_only_waits_for_its_answer(idle_radio):
    ...
    client.get("/api/hub/network/interface/wifi/scan?name=wlp3s0")
...
def test_a_radio_publishing_a_network_is_not_scanned(idle_radio, monkeypatch):
    ...
    response = client.get("/api/hub/network/interface/wifi/scan?name=wlp3s0")
