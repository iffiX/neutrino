...
router = APIRouter(
    prefix="/api/hub/network", tags=["network"], dependencies=[Depends(require_session)]
)
...
@router.post("/set", response_model=NetworkView)
...
@router.post("/mode/set", response_model=NetworkView)
...
async def update_interface(
    ...
    Args:
        settings: The interface, by name, and its new configuration.
        runtime: The shared runtime.
...
@router.post("/interface/remove", response_model=NetworkView)
async def delete_interface(
    request: InterfaceRequest, runtime: PanelRuntime = Depends(get_runtime)
) -> NetworkView:
...
@router.get("/wifi_network", response_model=SavedNetworkListView)
...
@router.post("/wifi_network/leave", response_model=SavedNetworkListView)
async def forget_network(
    request: WifiNetworkRequest, runtime: PanelRuntime = Depends(get_runtime)
) -> SavedNetworkListView:
...
@router.get("/interface/wifi/scan", response_model=WifiScanView)
def scan_wifi(name: str, runtime: PanelRuntime = Depends(get_runtime)) -> WifiScanView:
    ...
    Args:
        name: Interface name, from the query.
...
@router.post("/interface/wifi/join", response_model=NetworkView)
async def join_wifi(
    ...
    Args:
        request: The interface, the network to join and, if needed, its
            passphrase.
        runtime: The shared runtime.
