"""The gateway block of the AI page: its status, keys, usage, and applying.

The upstream providers themselves are edited on the same page, against
``/api/hub/ai``; a change there takes effect when this block's apply runs, which
...
from neutrino_hub.web.models import (
    ...
    CliproxyApiKeyRequest,
    CliproxyApiKeyView,
    CliproxyApiLoginCode,
    CliproxyApiLoginRequest,
...
@router.get("/gateway", response_model=CliproxyApiStatusView)
...
@router.get("/gateway/usage", response_model=CliproxyApiUsageView)
def usage(
...
@router.get("/gateway/journal", response_model=CliproxyApiJournalView)
def journal(
...
@router.post("/gateway/key/add", response_model=CliproxyApiStatusView)
...
@router.post("/gateway/key/remove", response_model=CliproxyApiStatusView)
def delete_key(
    request: CliproxyApiKeyRequest, runtime: PanelRuntime = Depends(get_runtime)
) -> CliproxyApiStatusView:
...
@router.post("/gateway/account/remove", response_model=CliproxyApiAccountsView)
def delete_account(request: CliproxyApiAccountRequest) -> CliproxyApiAccountsView:
    """Sign an account out; whatever it was serving stops now.

    Args:
        request: The account's token file, as the list reports it.
    ...
    Raises:
        HTTPException: 404 with ``unknown_account``, 502 with
            ``gateway_unreachable``.
    """
    _account_call(lambda client: client.delete_account(request.name))
    return read_accounts()


@router.post("/gateway/account_login/start", response_model=CliproxyApiLoginView)
...
@router.get("/gateway/account_login", response_model=CliproxyApiLoginStateView)
...
@router.post(
    "/gateway/account_login/code/set", response_model=CliproxyApiLoginStateView
)
def submit_login_code(request: CliproxyApiLoginCode) -> CliproxyApiLoginStateView:
    ...
    Args:
        request: The handle the start returned, and the code, or the whole
    ...
    """
    _account_call(lambda client: client.submit_code(request.state, request.code))
    return read_login(request.state)


@router.post("/gateway/account_login/stop", status_code=status.HTTP_204_NO_CONTENT)
def cancel_login(request: CliproxyApiLoginRequest) -> None:
    """Drop a login nobody finished.

    Args:
        request: The handle the start returned.

    Raises:
        HTTPException: 404 with ``login_expired`` when the gateway has already
            forgotten it, 502 with ``gateway_unreachable``.
    """
    _account_call(lambda client: client.cancel_login(request.state))
