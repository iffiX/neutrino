...
foreach ($line in Get-Content C:\ui_cmds.txt) {
  ...
  if ($parts[0] -eq 'cli') { & "$env:ProgramFiles\Neutrino Client\nclient.exe" ($parts[1] -split ' ') 2>&1 | Out-String; Write-Output "exit $LASTEXITCODE"; continue }
  if ($parts[0] -eq 'clibg') { Start-Process -FilePath "$env:ProgramFiles\Neutrino Client\nclient.exe" -ArgumentList ($parts[1] -split ' ') -WindowStyle Hidden -RedirectStandardOutput C:\out\clibg.txt; continue }
